...
import styles from './EasyApprovalDocumentForm.module.scss';
import { IEasyApprovalDocumentFormProps } from './IEasyApprovalDocumentFormProps';
...
export default class EasyApprovalDocumentForm extends React.Component<IEasyApprovalDocumentFormProps, CIState > {
  constructor(props : IEasyApprovalDocumentFormProps) {
    super(props);
  ...
  public render(): React.ReactElement<IEasyApprovalDocumentFormProps> {
    ...
    return (
      ...
      <div className={styles.easyApprovalDocumentForm}>
        <div className={styles.container}>
          ...
          <select id="ddlDepartment" title="Select Department" className='form-control form-control-sm' placeholder="Select Department">
         ...
         <div  className={styles.formrow+" "+"form-group row"} id="divClient">
         <div className='col-md-2 col-lg-2 col-sm-4 pr-0'>
        <div className={styles.lbl+" "+styles.Reqdlabel}>Do you want to add Client?</div>
          </div>
          <div className='col-md-10 col-lg-10 col-sm-8'>
    ...
    <label className="custom-radio" style={{padding:"8px"}}>
      ...
      <span className={"custom-control-description"}>No</span>
            ...
            <div className={styles.formrow+" "+"form-group row"}>
            <div className='col-md-2 col-lg-2 col-sm-4'>
          <label className={styles.lbl+" "+styles.Tcolumn}>Comments</label>
          </div>
          <div className='col-md-10 col-lg-10 col-sm-8'>
              <textarea  id="txtComments" className='form-control form-control-sm' style={{resize:'none'}}/>
          </div>         
             </div>
                ...
                <div className={styles.lbl + ' '  + "col-md-10 col-lg-10 col-sm-8"}>
                <PeoplePicker
           ...
           <h3 className="text-left"  style={{backgroundColor:"#50B4E6",fontSize:"16px", padding:'5px 10px', width:'100%', color:'#fff'}}>Attachments</h3>
            </div>
          ...
          {this.state.Note.map((vals)=>{
                let filename=vals.split("/")[1];
                                return (<span style={{position:"relative",padding:"5px"}}><a href={this.state.Absoluteurl+"/DocAttachments/"+vals}>{filename}</a><img src={Delete} style={{width:"10pt",height:"10pt",position:"absolute" }} onClick={() => this.DeleteNote(vals)}></img> </span>);
          ...
          {this.state.attachments.map((vals)=>{
                let filename=vals.split("/")[1];
                                return (<span style={{position:"relative",padding:"5px"}}><a href={this.state.Absoluteurl+"/DocAttachments/"+vals}>{filename}</a><img src={Delete} style={{width:"10pt",height:"10pt",position:"absolute" }} onClick={() => this.DeleteAttachment(vals)}></img> </span>);
       ...
       <div  className={styles.formrow +" "+ "form-group row m-0"}>
       
            <div id="btnCreate" style={{display:"block"}} > 
            <PrimaryButton className='btn' style={{width:"25pt",backgroundColor:"#0c78b8" , color:'#fff', marginLeft:'10px'}} text="Submit" onClick={() => { this.validateForm(); }} /> </div>
            ...
            <div id="btnClose" style={{display:"none",width:"25pt", marginLeft:'10px'}}>
...
// });
  ...
  private createItem(): void {
    ...
    let title="Doc/"+deptAlias+"/"+fulldate+"/"+counter.toString();
    ...
    let log='Submitted to '+this.state.MgrName+' by '+this.state.name+' on '+createDate;
        ...
        let approverID= this.state.userManagerIDs;
           let web=new Web('Main');           
        web.lists.getByTitle('DocWorkflow').items.add({
          Title:title,
          SeqNo:this.state.seqno,
          Subject:Subj,
          Department:dept,
          Comments:Comment,
            RecipientsId:{results:approverID},
          ...
          AuditLog:log
              ...
              let id=iar.data.ID;
              pnp.sp.site.rootWeb.lists.getByTitle("DocWorkflow").items.add({
                Title:title,
                SeqNo:this.state.seqno,
                Subject:Subj,
                PID:id,
  ...
  //   let num: number[] = [];
  //    return pnp.sp.site.rootWeb.lists.getByTitle('Departments').items.select("ID,Title,Dept_Alias,DocCounter,GroupID").filter("Dept_Alias eq '"+dept+"'").orderBy("ID asc").getAll().then((items: any[]) => {
  //     //debugger;;
  //      // console.log(items);    
  //      num[0]=parseInt(items[0].DocCounter)+1;
  //      num[1]=items[0].ID;
  ...
  let num: number[] = [];
    //return pnp.sp.site.rootWeb.lists.getByTitle('Departments').items.select("ID,Title,Dept_Alias,DocCounter,GroupID").filter("Dept_Alias eq '"+dept+"'").orderBy("ID asc").getAll().then((items: any[]) => {
      return pnp.sp.site.rootWeb.lists.getByTitle('Counter').items.select("ID,Title,NoteId,MemoId,Department,DocCounter").orderBy("ID asc").getAll().then((items: any[]) => {
    //debugger;;
      // console.log(items);    
      num[0]=parseInt(items[0].DocCounter)+1;
      num[1]=items[0].ID;
  ...
  // self.close();
     ...
     public DeleteAttachment(vals : String):void{
       ...
       let web=new Web('Main'); 
       let url=sitename+'/DocAttachments/'+vals;
       let fldr=vals.split("/")[0];
       let fldURL=sitename+'/DocAttachments/'+fldr;
     ...
     public DeleteNote(vals : String):void{
      ...
      web.getFileByServerRelativeUrl(url).recycle().then(data=> {  
        ...
        web.getFolderByServerRelativeUrl(fldURL).files.get().then((result) => {
          ...
          let test = document.getElementById("NoteDel");
          if(test)
          {
            (test as HTMLInputElement).value ="none"; 
          }
    ...
    // var uploadFlag : boolean = true;    
     public  AttachLib=(event : any)=> {
      ...
      let match = (new RegExp('[~#%\&{}+\|]|\\.\\.|^\\.|\\.$')).test(file.name.split(".")[0]);
       ...
       if (file!=undefined || file!=null){
                  ...
                  web.getFolderByServerRelativePath(listName).folders.add(fldr).then((data : any) => {  
     ...
     web.getFolderByServerRelativePath(listName+"/"+fldr).files.add(file.name, file, true).then((result : any) => {
