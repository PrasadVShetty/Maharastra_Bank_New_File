...
export default class EasyApprovalMemoForm extends React.Component<IEasyApprovalDocumentFormProps, CIState> {
    ...
    return (
      ...
      <div className={styles.easyApprovalDocumentForm}>
        <div className={styles.container}>
          ...
          <div id="divHeadingNew" className={styles.Mcolumn} style={{display:"block", backgroundColor: "#0c78b8", color:'#fff'}}>
          <h3 style={{fontSize: "18px", textAlign: "center", color: "white", padding:'5px 0px'}}>Document Form </h3> 
            
          </div>
                  
          <div hidden id="divHeadingSubmit" className="ms-Grid-col ms-u-sm10 block" style={{display:"none", backgroundColor: "#0c78b8", color:'#fff'}}>
          <h3   style={{fontSize:"18px",textAlign:"center",color:"white", padding:'5px 0px'}}>Document Form </h3> 
          </div>
          </div>
          ...
          {/* <div className={styles.formrow}>
          <table  style={{width:"100%",backgroundColor:"#50B4E6",textAlign:"left"}} className="table table-bordered"><tr>
      ...
      <div className="col-md-9" id="divDepartment">
         ...
         <div className={styles.formrow+" "+"form-group row"}>
          ...
          <div className="col-md-9" id="divSubject">              
             ...
             <div  className={styles.formrow+" "+"form-group row"}  id="divClientName" style={{display:"none"}}>
          ...
          <div className="col-md-9" id="divComments">             
                  ...
                  <div className={styles.formrow+" "+"form-group row"}>
                      <div className={styles.lbl+" " + "col-md-2"}>Recipients</div>
           ...
           <h3 className="text-left"  style={{backgroundColor:"#50B4E6",fontSize:"16px", padding:'5px 10px', color:'#fff'}}>Attachments</h3>
           </div> 
           </div>
          ...
          {this.state.Note.map((vals)=>{
                let filename=vals.split("/")[1];
                                return (<span style={{position:"relative",padding:"5px"}}><a href={this.state.Absoluteurl+"/DocAttachments/"+vals}>{filename}</a></span>);
          ...
          <div className={styles.formrow+" "+"form-group row"}>
      <div className="col-md-12" >
        <h3 className="text-left"  style={{backgroundColor:"#50B4E6",fontSize:"16px", padding:'5px 10px', color:'#fff'}}>Audit Log</h3>
          </div>
          </div>                                     
       ...
       <div  className={styles.formrow+" "+"form-group row"}>
            ...
            <div id="btnClose" style={{display:"none"}}>
               <PrimaryButton style={{width:"25pt",borderRadius:"5%",backgroundColor:"#50B4E6"}} text="Close" onClick={() => { this.cancel(); }} />
            </div>
            </div>
            <div>
...
if (tdDate) {
 ...
 private setFields(uid:number){
  ...
  //let fldr='';
  web.lists.getByTitle('DocWorkflow').items.select("Title,Department,Subject,SeqNo,Comments,FileName,DeptAlias,ClientName,AuditLog,Requester/ID,Requester/EMail,Recipients/Title,Recipients/ID,Recipients/EMail").expand('Recipients,Requester').filter('ID eq '+uid).orderBy("ID asc").getAll().then((items: any[]) => {
   ...
   $("#tdTitle").text(items[0].Title);
    ...
    let recp='';
    for(let i=0;i<items[0].Recipients.length;i++){
      recp+=items[0].Recipients[i].Title+"; ";
    }
    $('#divRecipient').text(recp);
if(items[0].ClientName!=null){
  $('#divClient').text(items[0].ClientName);
//   document.getElementById('divClientName').style.display='block';
const divClientName = document.getElementById('divClientName');
if (divClientName) {
  divClientName.style.display = 'block';
}
}
...
this.off();
 ...
 private getMainNote(){
  let web=new Web('Main');  
  let fldURL='DocAttachments/'+this.state.seqno;
...
private getAnnexures(){
  let web=new Web('Main');  
  let fldURL='DocAttachments/'+this.state.seqno+"-Annex";
  web.getFolderByServerRelativeUrl(fldURL).files.get().then((result) => {
    let links:any[]=[];
  ...
  private validateForm(): void {
   ...
   //else if(Subject.length>250){
    ...
    return;
     ...
     let CYes = document.getElementById('txtClient');
        if (CYes) {
        CYes.focus();
        }      
      _allowCreate = false;
  ...
  private createItem(): void {
    ...
    let deptAlias=jQuery('#ddlDepartment option:selected').val() as string;
    let counter=0;
    let uid=0;
      
    let filename=this.state.Notefilename;
   
     this.getCounter(deptAlias).then((countVal)=>{
  ...
  // self.close();
     ...
     public DeleteNote(vals:string):void{
      ...
      web.getFileByServerRelativeUrl(url).recycle().then(data=> {  
        ...
        const noteDel = document.getElementById("NoteDel");
        if (noteDel) {
        noteDel.style.display = "none";
        }
        const fileUploadInput = document.getElementById("fileUploadInput") as HTMLInputElement | null;
        if (fileUploadInput) {
   ...
   event.preventDefault();
    ...
    //  private Radibtnchangeevent(name,value){debugger;
    // debugger;
