...
export default class PNoteFormsEdit extends React.Component<IPaperlessApprovalProps, EditState> {
  ...
  private retrieveApprovers() {
    ...
    let data: any[] = [];
    let web = new Web('Main');
    web.lists.getByTitle('FApprovals').items.select("ID,Title,AppName,Status,AppEmail,Created,Modified,Seq").filter("Title eq '" + title + "'").orderBy("Seq asc").getAll().then((items: any[]) => {
      debugger;
  ...
  private retrieveRecommenders() {
    ...
    let data: any[] = [];
    let web = new Web('Main');
    web.lists.getByTitle('Approvals').items.select("ID,Title,AppName,Status,AppEmail,Created,Modified,Seq").filter("Title eq '" + title + "'").orderBy("Seq asc").getAll().then((items: any[]) => {
      debugger;
  ...
  private retrieveApprover(): Promise<any[]> {
    ...
    return web.lists.getByTitle('FApprovals').items.select("ID,Title,AppName,AppEmail,Approver/ID,Approver/EMail").filter("Title eq '" + title + "'").expand("Approver").orderBy("ID asc").getAll().then((items: any[]) => {
      debugger;
      let indx = 0;
      if (items.length > 0) {
  ...
  private retrieveFirstRecommender(): Promise<any[]> {
    ...
    let approverID : Number[] = [];
    let web = new Web('Main');
    return web.lists.getByTitle('Approvals').items.select("ID,Title,AppName,AppEmail,Approver/ID,Approver/EMail").filter("Title eq '" + title + "'").expand("Approver").orderBy("ID asc").getAll().then((items: any[]) => {
      debugger;
      let indx = 0;
      for (let i = 0; i < items.length; i++) {
        if (items.length > 0) {
        ...
        pnp.sp.web.siteUsers.getById(this.state.UserID).groups.get().then((grps: any) => {
          ...
          if (items[0].Status == 'Approved' || items[0].Status == 'Closed') {
            ...
            if ((CAppEmail == this.state.UserEmail && statusno == 1) || (CAppEmail == this.state.UserEmail && statusno == 6)) {
              ...
              web1.lists.getByTitle('RApprovals').items.select('Title', 'ID', 'AppEmail').filter("Title eq '" + seqno + "' and Status eq 'Referred'").getAll().then((RApprovalsItems: any[]) => {
            ...
            if (Appid[0] != 999) {
              ...
              this.retrieveApprover().then((Approverid) => {
                ...
                web.lists.getByTitle('Notes').items.getById(uid).update({
                  ...
                  pnp.sp.site.rootWeb.lists.getByTitle("Notes").items.select('Title', 'ID', 'PID').filter("PID eq " + uid).get().then(r => {
                    let Apprvrid = r[0].ID;
                    pnp.sp.site.rootWeb.lists.getByTitle('Notes').items.getById(Apprvrid).update({
                      CurApproverId: approverID,
                      CurApproverTxt: AppEmail,
                      Status: "Submitted to " + Appname + " (Approver#" + AppCount.toString() + ")"
                    }).then(() => {
  ...
  private async Approve() {
    ...
    try {
        ...
        if (Appid[0] !== 999) { // If there is a next approver
            ...
            await web.lists.getByTitle('Notes').items.getById(uid).update({
                CurApproverId: approverID,
                ApproversId: { results: Approvers },
                NotifyId: approverID,
                Migrate: "",
            ...
            let r = await pnp.sp.site.rootWeb.lists.getByTitle("Notes").items.select('Title', 'ID', 'PID').filter(`PID eq ${uid}`).get();
            let Approverid = r[0].ID;
            await pnp.sp.site.rootWeb.lists.getByTitle('Notes').items.getById(Approverid).update({
                CurApproverId: approverID,
                CurApproverTxt: AppEmail,
                StatusNo: 6,
                WorkflowFlag: "Triggered",
            ...
            if (Controller && ccID[0] !== curApprover) { // Assign Controller as approver
                ...
                if (ccID[0] === curApprover) {
                    let seqno = this.state.seqno;
                    let AppID = await web.lists.getByTitle('CApprovals').items.select('Title', 'ID', 'AppEmail').filter(`Title eq '${seqno}'`).get();
                    let NoteAppID = AppID[0].ID;
                    await web.lists.getByTitle('CApprovals').items.getById(NoteAppID).update({ Status: 'Approved' });
                }
              ...
              pnp.sp.site.rootWeb.lists.getByTitle('Notes').items.getById(Approverid).update({
                ...
                web.lists.getByTitle(listname).items.select('Title', 'ID', 'AppEmail').filter("Title eq '" + seqno + "' and AppEmail eq '" + UserEmail + "'").get().then(AppID => {
                  ...
                  web.lists.getByTitle(listname).items.getById(NoteAppID).update({
                    ...
                    web.lists.getByTitle('RApprovals').items.add({
                      Title: this.state.seqno,
                      Status: 'Pending',
                      Seq: 1,
    ...
    if (Comments.length > 0 && Comments.length < 2000) {
      ...
      this.AddAppComments().then(() => {
        this.checkRefApprover(ReturnById).then((App) => {
          ...
          if (App == 'Approver') { statusno = 1; listname = 'Approvals'; }
          else {
            ...
            web.lists.getByTitle('Notes').items.getById(uid).update({
              ...
              pnp.sp.site.rootWeb.lists.getByTitle("Notes").items.select('Title', 'ID', 'PID').filter("PID eq " + uid).get().then(r => {
                ...
                pnp.sp.site.rootWeb.lists.getByTitle('Notes').items.getById(Approverid).update({
                  ...
                  web.lists.getByTitle("RApprovals").items.select('Title', 'ID', 'Seq', 'Approver/ID').expand('Approver').filter("Title eq '" + seqno + "' and Approver/ID eq " + userid + " and Status eq 'Pending'").get().then(rapp => {
                    let Refid = rapp[0].ID;
                    web.lists.getByTitle('RApprovals').items.getById(Refid).update({
      ...
      this.AddAppComments().then(() => {
        this.checkRefApprover(ReturnById).then((App) => {
          ...
          if (App == 'Approver') { statusno = 1; listname = 'Approvals'; }
            ...
            if (this.state.ccIDS.length > 0) {
              if (this.state.ccIDS[0] == ReturnById) { listname = 'CApprovals'; }
            }
          }
              ...
              pnp.sp.site.rootWeb.lists.getByTitle('Notes').items.getById(Approverid).update({
                ...
                web.lists.getByTitle(listname).items.select('Title', 'ID', 'Seq', 'Approver/ID').expand('Approver').filter("Title eq '" + seqno + "' and Approver/ID eq " + ReturnById).get().then(rapp => {
                  ...
                  web.lists.getByTitle(listname).items.getById(Refid).update({
                    ...
                    if (curruser != req) {
                      web.lists.getByTitle('Approvals').items.select('Title', 'ID', 'Seq', 'Approver/ID').expand('Approver').filter("Title eq '" + seqno + "' and Approver/ID eq " + curruser).get().then(retapp => {
                        let Returnid = retapp[0].ID;
                        web.lists.getByTitle('Approvals').items.getById(Returnid).update({
                          Status: "Returned Back",
  ...
  private checkRefApprover(appID: number): Promise<string> {
    ...
    let App = '';
    let web = new Web('Main');
    return web.lists.getByTitle('Approvals').items.select("ID,Title,AppName,AppEmail,Approver/ID").expand('Approver').filter("Title eq '" + title + "' and Approver/ID eq " + appID).orderBy("Seq asc").getAll().then((items: any[]) => {
  ...
  private retrieveRefers() {
    ...
    let data: any[] = []; 
    let web = new Web('Main');
    web.lists.getByTitle('RApprovals').items.select("ID,Title,AppName,Status,AppEmail,Created,Modified,Author/Title,Seq").expand('Author').filter("Title eq '" + title + "'").orderBy("Seq asc").getAll().then((items: any[]) => {
      debugger;
  ...
  private retrieveController() {
    ...
    let data: any[] = []; 
    let web = new Web('Main');
    web.lists.getByTitle('CApprovals').items.select("ID,Title,AppName,Status,AppEmail,Created,Modified,Author/Title,Seq").expand('Author').filter("Title eq '" + title + "'").orderBy("Seq asc").getAll().then((items: any[]) => {
      debugger;
  ...
  private retrieveRecommendersNew() {
    ...
    let web = new Web('Main');
    let userID = this.state.UserID;
    web.lists.getByTitle('Approvals').items.select("ID,Title,Status,Modified,AppName,Author/ID").expand('Author').filter("Title eq '" + title + "'").orderBy("Seq asc").getAll().then((items: any[]) => {
      debugger;
      if (items.length > 0) {
        for (let i = 0; i < items.length; i++) {
  ...
  private AddNotesNotifications(status: string, approverID: number): Promise<any[]> {
    ...
    return web.lists.getByTitle("NotesNotifications").items.add({
      Title: title,
      SeqNo: seqno,
      Subject: Subj,
      Department: dept,
