...
import * as strings from 'EasyApprovalNoteFormWebPartStrings';
import EasyApprovalNoteForm from './components/EasyApprovalNoteForm';
import { IEasyApprovalNoteFormProps } from './components/IEasyApprovalNoteFormProps';
...
export interface IEasyApprovalNoteFormWebPartProps {
  description: string;
}

export default class EasyApprovalNoteFormWebPart extends BaseClientSideWebPart<IEasyApprovalNoteFormWebPartProps> {
 ...
 if(qstrPid.length>1){qstringPid=qstr[1];}
   if(qstr.length>1){qstring=qstr[1];}
    const element: React.ReactElement<IEasyApprovalNoteFormProps > = React.createElement(
