...
export default class PNoteForms extends React.Component<IPaperlessApprovalProps, CIState> {
  ...
  private retrieveController() {
    ...
    let web = new Web('Main');
    web.lists.getByTitle('CApprovals').items.select("ID,Title,AppName").filter("Title eq '" + title + "' and AppID eq " + ControllerID[0]).orderBy("Seq asc").getAll().then((items: any[]) => {
      //debugger;
      if (items.length > 0) {
        for (let i = 0; i < items.length; i++) {
      ...
      if (Recommenders > 0) {
        this.retrieveFirstRecommender().then((Appid) => {          
          ...
          web.lists.getByTitle('Notes').items.add({
            ...
            console.log(iar.data.ID);
            let id = iar.data.ID;
            pnp.sp.site.rootWeb.lists.getByTitle("Notes").items.add({
              Title: title,
              Subject: Subj,
              Department: dept,
              NoteType: Financial,
