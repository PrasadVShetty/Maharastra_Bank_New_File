...
import _styles from './EasyApprovalAllNotesChecklistView.module.scss';
import { IEasyApprovalAllNotesChecklistViewProps } from './IEasyApprovalAllNotesChecklistViewProps';
...
export default class EasyApprovalAllNotesChecklistView extends React.Component<IEasyApprovalAllNotesChecklistViewProps, IReactPnpResponsiveDataTableState> {
  constructor(props: IEasyApprovalAllNotesChecklistViewProps, state: IReactPnpResponsiveDataTableState) {  
    //debugger;
    super(props);  
    this.state = {  
      Sitename:'',
  ...
  //pnp.sp.web.lists.getByTitle('Notes').items.select('ID,Title,Status,Department,Subject,PID,Created,ClientName,Modified,Requester/Title,Sitename,CurApprover/EMail,CurApprover/Title').expand('Requester,CurApprover').orderBy("Modified",false).top(5000).get().then(r => {  
    pnp.sp.web.lists.getByTitle('ChecklistNote').items.select('ID,Title,Status,Department,Subject,PID,Created,ClientName,Modified,Requester/Title,Sitename,CurApprover/EMail,CurApprover/Title').expand('Requester,CurApprover').orderBy("Modified",false).top(5000).get().then(r => {  
  ...
  public render(): React.ReactElement<IEasyApprovalAllNotesChecklistViewProps> {
    //debugger;
    return (
      <div>  
        <h2 id="Submitted" style={{textAlign:"center",backgroundColor:"#0c78b8",color:"white",display:"block",  padding:'5px 0px', fontSize:'18px'}}>View - All Checklist Notes </h2>
        <div className='table-reponsive' style={{overflowX:"auto",width: "100%"}}>               
            <table className='table table-striped table-bordered row-border stripe' id='MNSpfxDatatableNew'>  
          <thead>  
            <tr>  
              <th>Title </th>  
