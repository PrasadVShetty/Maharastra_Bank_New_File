...
import _styles from './EasyApprovalMyPendingNotesChecklistView.module.scss';
import { IReactPnpResponsiveDataTableState } from './DataTableState';
import { IEasyApprovalMyPendingNotesChecklistViewProps } from './IEasyApprovalMyPendingNotesChecklistViewProps';
...
export default class EasyApprovalMyPendingNotesChecklistView extends React.Component<IEasyApprovalMyPendingNotesChecklistViewProps, IReactPnpResponsiveDataTableState> {
  constructor(props: IEasyApprovalMyPendingNotesChecklistViewProps, state: IReactPnpResponsiveDataTableState) {
    debugger;
    super(props);
    this.state = {
      Sitename: '',
  ...
  private fetchdatas(CurrUserEmail: any) {
    ...
    let SiteUrl = this.props.siteUrl;
    //let URL = SiteUrl + "/_api/Web/Lists/GetByTitle('Notes')/items?$select=ID,Title,Status,Subject,PID,Created,ClientName,Modified,Requester/Title,Requester/EMail,Sitename,CurApprover/EMail,CurApprover/Title&$expand=Requester,CurApprover&$orderby=Modified desc&$top=5000";
    let URL = SiteUrl + "/_api/Web/Lists/GetByTitle('ChecklistNote')/items?$select=ID,Title,Status,Subject,PID,Created,ClientName,Modified,Requester/Title,Requester/EMail,Sitename,CurApprover/EMail,CurApprover/Title&$expand=Requester,CurApprover&$orderby=Modified desc&$top=5000";
  ...
  public render(): React.ReactElement<IEasyApprovalMyPendingNotesChecklistViewProps> {
    return (
      <div>
        ...
        <table className='table table-striped table-bordered row-border stripe' id='SpfxDatatableNew'>
          <thead>
            <tr>
              <th>Title</th>
              <th>Requester</th>
