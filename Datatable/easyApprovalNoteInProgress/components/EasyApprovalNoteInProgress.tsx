...
export default class EasyApprovalNoteInProgress extends React.Component<IEasyApprovalNoteInProgressProps, IReactPnpResponsiveDataTableState> {
 ...
 public componentDidMount(){  
  ...
  pnp.sp.web.lists.getByTitle('Notes').items.select('ID,Title,Status,Subject,Department,PID,Created,ClientName,Modified,Requester/Title,Sitename,CurApprover/EMail,CurApprover/Title').expand('Requester,CurApprover').filter(filterText).orderBy("Modified",false).top(5000).get().then(r => {  
